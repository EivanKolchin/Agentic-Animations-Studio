...
import { BIBLE, CANON } from './env.mjs'
 ...
 *
 * Resolved against the BIBLE's folder, not the process working directory.
 * The first version left anything beginning with a dot alone, which made
 * `../canon/fox/source.webp` mean something different depending on where
 * the CLI happened to be run from - and the canon is exactly the thing an
 * asset most wants to point at from the bible.
 */
export function referencesFor(bible, asset) {
  const refs = [...list(bible.references), ...list(asset.references)]
  const out = refs.map((r) => (isAbsolute(r) ? r : resolve(bible.dir, r)))

  /**
   * `canon: 'fox'` attaches that character's one true generation.
   *
   * Measured rather than assumed: six candidates of the same fox, three
   * with the master alone and three with the canon beside it, judged
   * against the same standard. The three carrying the canon came back at
   * 10 on model where the others sat at 9, and two of them are hard to
   * tell from it at a glance.
   *
   * It is a NAME rather than a path because the point is that nobody has
   * to remember to attach it. A spec that has to spell out
   * ../canon/fox/source.webp is a spec that will one day not, and then a
   * character drifts for a reason nobody can see in the diff.
   */
  for (const name of list(asset.canon)) {
    const f = ['source.webp', 'source.png'].map((x) => join(CANON, name, x)).find(existsSync)
    if (!f) {
      throw new Error(
        `Asset "${asset.id}" names canon "${name}", but there is no ${join(CANON, name)}/source.*\n` +
          'A character has to be generated and promoted before anything can be asked to match it.',
      )
    }
    out.push(f)
  }
